...
let
  ...
  # Normaliza cfg.kernel para sempre retornar um atributo set
  # Se for true, ativa todas as opções. Se for false, desativa todas.
  # Se for um set, usa os valores específicos.
  kernelCfg =
    if isBool cfg.kernel then
      {
        enable = cfg.kernel;
        params = cfg.kernel;
        sysctl = cfg.kernel;
        transparent_hugepage = cfg.kernel;
      }
    else
      cfg.kernel;
in
{
  # Opções para kernel-settings
  options.velora = {
    kernel = mkOption {
        ...
        types.submodule {
          options = {
            enable = mkOption {
              type = types.bool;
              default = false;
              description = "Ativa todas as configurações do kernel (params + sysctl + transparent_hugepage).";
            };
            params = mkOption {
              type = types.bool;
              default = false;
              description = "Ativa os parâmetros de boot do kernel (kernelParams).";
            };
            sysctl = mkOption {
              type = types.bool;
              default = false;
              description = "Ativa as configurações de sysctl do kernel em tempo de execução.";
            };
            transparent_hugepage = mkOption {
              type = types.bool;
              default = false;
              description = "Ativa as configurações de transparent_hugepage do kernel em tempo de execução.";
            };
          };
        }
      );
      default = false;
      description = ''
        Configurações de otimização do kernel.

        Pode ser usado de duas formas:
        1. Boolean simples: `velora.kernel = true;` (ativa tudo)
        2. Granular: `velora.kernel.params = true;` (ativa apenas params)
      '';
    };
  };

  # Implementação das kernel-settings
  config = mkMerge [
    (mkIf (kernelCfg.transparent_hugepage || kernelCfg.enable) {
    ...
    # Kernel Params - aplicados somente se params for true
    (mkIf (kernelCfg.params || kernelCfg.enable) {
      boot.kernelParams = [
        # Desativa mitigações de segurança de CPU (Spectre, Meltdown, etc.)
        # ⚠️ SEGURANÇA: Aumenta performance mas remove proteções contra vulnerabilidades de CPU
        # Impacto: +5-30% performance dependendo da carga de trabalho
        # "mitigations=off"

        # Desativa os watchdog timers do kernel
        # Efeito: Reduz consumo de energia e overhead, acelera boot/shutdown
        # Impacto: Sistema não será reiniciado automaticamente em caso de travamento
        "nowatchdog"

        # Sempre usa páginas de memória grandes (2MB ao invés de 4KB)
        # Efeito: Reduz overhead de gerenciamento de memória, melhora performance de cache
        # Impacto: Melhor performance em aplicações que usam muita RAM (games, VMs, compilação)
        "transparent_hugepage=always"

        # Ignora controle de energia do processador via ACPI
        # Efeito: Desativa throttling de CPU imposto pelo firmware
        # Impacto: CPU opera em frequências mais altas constantemente
        "processor.ignore_ppc=1"

        # Permite escrita no embedded controller via /sys/kernel/debug/ec
        # Efeito: Habilita controle de ventoinhas e sensores via software
        # Impacto: Necessário para ferramentas de controle de cooling em laptops
        "ec_sys.write_support=1"

        # Desativa carregamento automático do KVM no boot
        # Efeito: KVM só é carregado quando explicitamente necessário
        # Impacto: Boot ligeiramente mais rápido, menor consumo de memória
        "kvm.enable_virt_at_load=0"

        # Usa Block Multi-Queue para dispositivos SCSI
        # Efeito: Paraleliza operações de I/O em múltiplos cores
        # Impacto: Melhor performance em SSDs NVMe, menor latência
        "scsi_mod.use_blk_mq=1"

        # Define o scheduler de I/O como 'noop' (sem fila de prioridade)
        # Efeito: Passa requisições direto para o dispositivo sem reordenação
        # Impacto: Ideal para SSDs que já fazem seu próprio gerenciamento
        "elevator=noop"

        # Habilita preempção total do kernel
        # Efeito: Permite que tarefas sejam interrompidas a qualquer momento
        # Impacto: Menor latência, melhor responsividade (ideal para desktop/gaming)
        "preempt=full"
      ];
    })

    # Sysctl - aplicados somente se sysctl for true
    (mkIf (kernelCfg.sysctl || kernelCfg.enable) {
      boot.kernel.sysctl = {
